...
//     CLDNN_ERROR_GREATER_THAN(node.id(), "Negate input offset spatial Y", -input_offset.spatial[1], "input window size spatial Y", filter_size.spatial[1], "First convolution is outside of image. please reduce input offset Y");
    ...
    if (input_layout.format == format::bfzyx) {
        // convolution 3D
        CLDNN_ERROR_LESS_OR_EQUAL_THAN(node.id(), "Stride spatial Z", stride.spatial[2], "value", 0, "Stride spatial Z must be positive (>= 1)");
        CLDNN_ERROR_LESS_OR_EQUAL_THAN(node.id(), "Dilatation spatial Z", dilation.spatial[2], "value", 0, "Dilatation spatial Z must be positive (>= 1)");
        CLDNN_ERROR_GREATER_THAN(node.id(), "Input offset spatial Z", 2 * input_offset.spatial[2], "input layout spatial Z", input_layout.size.spatial[1], "There is no input data to process");
    }
    ...
    if (desc->with_output_size)
    {
        ...
        CLDNN_ERROR_LESS_OR_EQUAL_THAN(node.id(), "User defined output spatial Z", desc->output_size.spatial[2], "value", 0, "must be positive(>= 1)");

        tensor output_size(input_layout.size.batch[0], desc->output_size.feature[0],
    ...
    : parent(network, node)
{
    ...
    for (decltype(split) j = 0; j < split; j++)
    {
        ...
        if (bias_term())
        {
            ...
            CLDNN_ERROR_NOT_EQUAL(node.id(), "Bias spatial[2]", bias_inst.size.spatial[2], "expected size of spatial[2]", 1, "Biases isn't 1D vector.");
