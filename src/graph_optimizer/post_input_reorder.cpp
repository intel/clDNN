...
*/
program_node& post_input_reorder::add_reorder(program_impl& p, program_node* node, program_node* usr, layout reorder_layout) {
        ...
        if (node->is_type<fully_connected>())
        {
            ...
            if (input_layout.format != layout_format)
            {
                layout current_layout(input_layout.data_type,
                    layout_format,
                    input_layout.size);
                node->set_output_layout(current_layout);
                auto& reorder = add_reorder(p, input, node, node->get_output_layout());
                reorder.get_output_layout();
