...
vector<float> load_data(const string& filename) { (void)filename; return{ 0 }; }

// Allocates memory and loads data from file.
// Memory layout is taken from file.
memory load_mem(const engine& engine, const string& filename) { (void)engine; (void)filename; return{ 0 }; }

// Load image, resize to [x,y] and store in a vector of floats
// in the order "bfyx".
vector<float> load_image_bfyx(const string& str, int x, int y) { (void)str; (void)x; (void)y; return{ 0 }; }
// //////////////////////////////////////////////////////////////
