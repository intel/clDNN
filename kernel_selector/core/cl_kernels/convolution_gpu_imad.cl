...
)
{
    ...
    if (1) {
        // feature maps are an array of slices, each H,W position within the slice contains four 8bit feature maps, packed like RGBA components into a 32 bit pixel. 
        int row_size_bytes = (_OW + OWPAD) * PACK;
        int slice_pack_size_bytes = row_size_bytes * (_OH + OHPAD); // slice_pack is a pack of 4 feature map tiles that are [OH][OW][4] that are stored within the full [N][C/4][H][W][4] output.
        int slice_pack_index = fm / PACK;  // dividing the feature map index by 4 gives us the slice_pack_index in each lane (each lane within block of 4 will have same index).
        int slice_pack_start_addr_bytes = slice_pack_index * slice_pack_size_bytes;  // each group of 4 simd lanes points to start of it's slice pack.
        int slice_pack_addr_bytes = slice_pack_start_addr_bytes + (lid % PACK);  // // make each lane within the group of 4(PACK) simd lanes point to an individual byte witihn the uchar4 at start of slice pack.
        slice_pack_addr_bytes += or * row_size_bytes + oc * PACK;  //adjust to particular tile that we are working on
