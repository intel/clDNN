														   ...
														   )
{
    ...
    // eltwise calibs
    float4 eltw_calib_f = as_float4(intel_sub_group_block_read4((__global uint*) (eltw_calibrations + feature) ));
    ...
    for (uint i = 0; i < SG_TILE_M / (sizeof(int8) / sizeof(int)); i++)
    {
        ...
        // do ELTWISE
        {
            ...
            for(uint s = 0; s < 8; s++)
            {
                sum[s] = (int)as_char(regC_uchar8[offset_uc8][s]) + (int)as_char(eltw_input2[s]);
            }
       		regC_uchar8[offset_uc8].s0 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s0)  * eltw_calib_f.s0)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s1 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s1)  * eltw_calib_f.s1)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s2 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s2)  * eltw_calib_f.s2)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s3 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s3)  * eltw_calib_f.s3)), NL_M_ELTW, NL_N_ELTW));

       		regC_uchar8[offset_uc8].s4 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s4)  * eltw_calib_f.s0)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s5 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s5)  * eltw_calib_f.s1)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s6 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s6)  * eltw_calib_f.s2)), NL_M_ELTW, NL_N_ELTW));
       		regC_uchar8[offset_uc8].s7 = as_uchar(ACTIVATION_ELTW( convert_char((int)round( (float)(sum.s7)  * eltw_calib_f.s3)), NL_M_ELTW, NL_N_ELTW));
        }
