...
namespace kernel_selector {
    ...
    namespace
    {
        std::shared_ptr<JitConstant> GetJit_GetIndexForDataLayout(std::string jitName, std::string prefix, DataLayout dataLayout)
        {
            std::string jitValue;
            switch (dataLayout)
            {
    ...
    KernelsData EltwiseKernel_mixed_byxf_and_fs_b_yx_fsv32::GetKernelsData(const Params& params, const optional_params& options) const
    {
        ...
        // in fs_b_yx_fsv32 format we will process 2 features per work item, so reads/writes are done in full writes for fp16
        ...
        if ((newParams.output.GetLayout() == newParams.inputs[0].GetLayout()) &&
            (newParams.output.GetLayout() == newParams.inputs[1].GetLayout())) //There is no need for reordering kernel, better use something more optimal
        {
