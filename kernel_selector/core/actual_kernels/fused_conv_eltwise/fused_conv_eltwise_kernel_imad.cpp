...
KernelsData fused_conv_eltwise_kernel_imad::GetKernelsData(const Params& params,
                                                             ...
                                                             const DispatchData& kd) const
{
    ...
    size_t obw, obh;
    getOutBlock_WH(
        iDims[iX].v, params.conv.stride.x, iDims[iX].pad.before + iDims[iX].pad.after, obw, obh);
    mem_consts.AddConstants(
        {MakeJitConstant("OUT_BLOCK_WIDTH", obw), MakeJitConstant("OUT_BLOCK_HEIGHT", obh)});
...
fused_conv_eltwise_kernel_imad::SetDefault(const fused_conv_eltwise_params& params, int) const
{
    DispatchData kd;
    ...
    size_t otw, oth;
    getOutBlock_WH(
        iDims[iX].v, params.conv.stride.x, iDims[iX].pad.before + iDims[iX].pad.after, otw, oth);
...
bool fused_conv_eltwise_kernel_imad::Validate(const Params& params, const optional_params& options) const
{
    ...
    else if ((newParams.conv.filterSize.x != m_FilterSizeX)
             || (newParams.conv.filterSize.y != m_FilterSizeY))
    {
